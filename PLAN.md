...
**Status:** Phasen 0–3 gebaut und im Browser geprüft. Nächster Schritt:
Phase 4 (Job-Engine).
**Stand:** 2026-08-07
...
| Architektur | Lokaler Node-Server + SQLite (`npm start` → `http://localhost:8787`) |
...
| `kein_fit` | Dunkelgrau `#475569` | Geschlossen, Kette, zu klein |
...
### Aktion A — „Analyse" (Online-Recherche)

Server startet:

```
claude -p "<Recherche-Prompt>" --output-format stream-json
  --json-schema <VenueSchema> --permission-mode acceptEdits
  --model sonnet
```
...
2. Strukturiertes JSON (via `--json-schema`) → schreibt direkt in die DB,
   Score wird neu berechnet, Pin-Ring wird massiv, Status → `researched`
...
### Aktion C — „Kontakt-Entwurf" (Phase 7)

Erzeugt aus der Analyse einen IG-DM-Entwurf und eine Mail-Variante,
personalisiert auf die Story des Ladens, mit Link zur Demo. **Erzeugt nur
Entwürfe — versendet nie selbst.**
...
|---|---|---|---|
| 0 | ✅ **Fundament** | erledigt | Express + SQLite, `npm start` → :8787 |
...
| 4 | **Job-Engine** | `Phase 4 bauen` | Queue, `claude -p`-Runner, Live-Log per SSE, Abbruch, Budget-Deckel |
| 5 | **Aktion Analyse** 🔎 | `Phase 5 bauen` | MD-Analyse + DB-Rückschreibung + Neu-Scoring, Tiefen-Scan im Batch |
| 6 | **Aktion Demo bauen** | `Phase 6 bauen` | `restaurant-website-build` verdrahtet, Ordner-Scan markiert die 10 bestehenden Demos |
| 7 | **CRM-Tiefe** | `Phase 7 bauen` | Kontakt-Historie, Wiedervorlage, Kontakt-Entwürfe, Funnel-Statistik |
| 8 | **Härtung & Skalierung** | `Phase 8 bauen` | Regions-Batch, Dedupe/Merge, Backup, 10k-Pins-Test, Google-Adapter scharf schaltbar, Doku |
...
**F1 — Vier unklare Ordner.** Gehören `loewen-pub`, `rebstock`, `vibes-wil`
und `Ilge-pianobar` zu Gastro-Betrieben, und wenn ja: welcher Status? Ohne
Antwort landen sie als `demo_gebaut` auf der Karte — nicht falsch, aber
vielleicht ungenau.
